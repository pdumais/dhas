...
other technologies such as interfacing my Asterisk PBX and accessing a [WebRelay device](http://www.controlbyweb.com/webrelay/). The software triggers events (such as lightswitch on/of etc...) by invoking a Lua script. So it is possible to 
control everything by modifying the Lua script instead of having to hardcode behaviours in the software.
...
Lua Script
==============
When DHAS loads, it will run the code from the Lua script (path defined in config.h)
...
When an event is generated by DHAS, the onEvent function will be invoked from the Lua script.
```
function onEvent(st)
...
Since the onEvent function receives a JSON string, you will need a Lua JSON decoder. I am using the one at
[http://regex.info/blog/lua/json](http://regex.info/blog/lua/json). The file is not distributed in this software

An example script can be found in the documentation folder.

Eents 
============== 
Not documented yet... please refer to the source code and the example Lua script.
...
The Makefile is customized for my system so you may have to modify it to make it more generic.
    ...
    - make updatescript: will copy data/script.lua to the install directory and kill -HUP the process
